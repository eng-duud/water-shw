...
export default function SingleBillPrint() {
  ...
  useEffect(() => {
    ...
    const fetchBill = async () => {
      try {
        const res = await fetch(`/api/billing/bill/${id}`); // We will create this API route or fetch from client
        // Wait, instead of calling /api/billing/bill/${id}, let's check if we can write a fast route, or query prisma directly.
        // In Next.js client-side we can query a general endpoint or we can retrieve it. Let's create the api endpoint `/api/billing/bill/[id]`
  ...
  useEffect(() => {
    if (bill) {
      // Small delay to make sure rendering is complete
  ...
  return (
    <div className="print-container bg-white p-6 max-w-lg mx-auto border border-slate-200 shadow-sm rounded-lg text-slate-800 space-y-6">
      {/* Header */}
      ...
      <div className="text-center pt-4 no-print">
        <button
          ...
          className="bg-brand-600 hover:bg-brand-700 text-white font-bold text-xs px-4 py-2 rounded-lg transition-colors"
        >
